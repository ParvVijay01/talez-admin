...
class DeliveryChargeSetupController extends Controller
{
    ...
    public function deliveryFeeSetup(Request $request): Factory|\Illuminate\Foundation\Application|View|Application
    {
        ...
        return view('admin-views.business-settings.delivery-fee', compact('branches'));
    }
    ...
    public function storeKilometerWiseDeliveryCharge(Request $request): RedirectResponse
    {
        $request->validate([
            'branch_id' => 'required',
            'delivery_charge_per_kilometer' => 'required|numeric|min:1|max:99999999',
    ...
    public function storeFixedDeliveryCharge(Request $request): RedirectResponse
    {
        $request->validate([
            'branch_id' => 'required',
            'fixed_delivery_charge' => 'required|numeric|min:0.1|max:99999999',
        ]);
    ...
    public function changeDeliveryChargeType(Request $request): JsonResponse
    {
        ...
        if ($status) {
            ...
            if (!$newDeliveryChargeType) {
                return response()->json(['error' => 'New delivery charge type must be selected when deactivating the current setup.'], 400);
            }
    ...
    public function StoreAreaWiseDeliveryCharge(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'delivery_charge' => 'required|numeric|min:0.01|max:99999999',
        ]);
    ...
    {
        $request->validate([
            'area_file' => 'required|mimes:xlsx,csv',
        ]);
        ...
        try {
            ...
            if (count($collections) < 1) {
                return response()->json([
                    'status' => 'error',
                    'message' => translate('The file is empty or no valid data is found.')
                ], 422);
            }
            ...
            foreach ($fieldArray as $field) {
                if (!array_key_exists($field, $firstRow)) {
                    return response()->json([
                        'status' => 'error',
                        'message' => translate('Invalid file format')
                    ], 422);
                }
            }
